...
using namespace std;
void xAna_hh3(std::string inputFile){
  ...
  TH1F* h_hh=new TH1F("h_hh","",nbin,xmin,xmax);
  h_hh->SetXTitle("M_{jj} [GeV]");
  h_hh->SetYTitle(Form("Events per %d GeV",(int)((xmax-xmin)/nbin)));
  TH1F* h_hh2=(TH1F*)h_hh->Clone("h_hh2");
  //Event loop
  ofstream fout;
  fout.open("eiko.dat");
  ...
  for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
    ...
    if(nSubBJet<3)continue;
    nPass[8]++;
    
    float tau21[2]={
      fatjetTau2[bindex1]/fatjetTau1[bindex1],
      fatjetTau2[bindex2]/fatjetTau1[bindex2]};
    ...
    if(nSubBJet==3 && tau21[0]>0.6)continue;
    if(nSubBJet==3 && tau21[1]>0.6)continue;
    nPass[9]++;

    if(nSubBJet==3)
      nPass[10]++;

    TLorentzVector* thish = (TLorentzVector*)fatjetP4->At(bindex1); 
    TLorentzVector* thath = (TLorentzVector*)fatjetP4->At(bindex2); 
