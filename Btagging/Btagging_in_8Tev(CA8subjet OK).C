...
void Btagging_in_8Tev()
{
	...
	int max_N_of_candidate_sub_nbjet=0;
  	for (Long64_t Nth = 0; Nth < Nev; Nth++)
		...
		vector< vector<CA8subjet> > this_CA8subjets=events[Nth].CA8subjets_deoverlap_with_lepton_and_subjet;
	...
	// event loop : get fraction ratio matrix
  	for (Long64_t Nth = 0; Nth < Nev; Nth++)
		...
		N_of_candidate_sub_nbjet=N_of_CA8subjet-N_of_candidate_sub_bjet;
		fraction_ratio[N_of_candidate_sub_bjet][N_of_candidate_sub_nbjet]+=(1/double(N_available_event));
	}	
