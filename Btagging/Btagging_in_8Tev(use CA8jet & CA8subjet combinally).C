...
// global variable================================================
	...
	double best_bcut_eff=0;
	double best_nbcut_eff=0;
	double best_eff=0;
	double best_mr=0;
...
class jet_for_btagging : public PO
{
	public:
	Float_t csv;
	string kind;
	...
	jet_for_btagging():PO(){}	
    	jet_for_btagging(Float_t pt_,Float_t eta_,Float_t phi_, Float_t en_, Float_t M_,
		Float_t csv_, string kind_)	
	{ pt=pt_;eta=eta_;phi=phi_;en=en_;M=M_;csv=csv_;kind=kind_;}
...
class event
{
	...
	vector< jet_for_btagging > jets_for_btagging;
	vector<ele> eles;
	vector<mu> mus;
	met missing_et;
	event(){}
...
bool is_pass_csvl(jet_for_btagging j_)
{
	if (j_.csv>0.244 && j_.csv<=1){return true;}
	else {return false;}
}
...
bool is_pass_csvm(jet_for_btagging j_)
{
	if (j_.csv>0.679 && j_.csv<=1){return true;}
	else {return false;}
}
...
void fcn_cut(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
	...
	double total_log_like=0;
        for (int k = 0; k < Nk_from_data.size(); k++)
        {
		// par[0] is bcut_eff, par[1] is nbcut_eff
		

		if(get_expvalue_of_Nk( par[0], par[1] ,k)==0){continue;}
		double this_log_like=log_poisson(double(Nk_from_data[k]), get_expvalue_of_Nk(par[0], par[1] ,k));
                total_log_like += this_log_like;
		if (this_log_like < -10000000)
		{ 
			cout<<"this_log_like < -10000000"<<"when bcut_eff="<<par[0]<<",nbcut_eff="<<par[1]<<",k="<<k;
			cout<<",Nk_from_data[k]="<<double(Nk_from_data[k])<<",get_expvalue_of_N="<<get_expvalue_of_Nk(par[0], par[1] ,k)<<endl;
		}  
        }
...
void fcn_btagging(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
	...
	double total_log_like=0;
        for (int t = 0; t < Nt_from_data.size(); t++)
        {
		// par[0] is eff, par[1] is mr
		if(get_expvalue_of_Nt(best_bcut_eff, best_nbcut_eff, par[0], par[1] ,t)==0){continue;}
		double this_log_like=log_poisson(double(Nt_from_data[t]), get_expvalue_of_Nt(best_bcut_eff, best_nbcut_eff, par[0], par[1] ,t));
                total_log_like += this_log_like;
		if (this_log_like < -10000000)
		{ 
			cout<<"this_log_like < -10000000"<<"when eff="<<par[0]<<",mr="<<par[1]<<",t="<<t;
			cout<<",Nt_from_data[t]="<<double(Nt_from_data[t])<<",get_expvalue_of_N="<<get_expvalue_of_Nt(best_bcut_eff, best_nbcut_eff, par[0], par[1] ,t)<<endl;
		}  
        }
...
void Btagging_in_8Tev()
{
	...
	// Btagging function
	typedef bool (*jet_alg)(jet_for_btagging);
	...
	// event loop : set up
  	for (Long64_t Nth = 0; Nth < Nev; Nth++)
		...
		// get CA8jet after deoverlap with other sub jet and good lepton in every event
			...
			//test
			if (subjet_overlap==true)
			{	
				jet_for_btagging j =  jet_for_btagging(this_j.pt, this_j.eta,this_j.phi,this_j.en,this_j.M,this_j.csv,"CA8jet");
				this_ev.jets_for_btagging.push_back(j);
			}
			else 
			{
				for (int j=0;j<CA8subjetPrunedPt->size();j++)
				{ 
					CA8subjet this_sj=this_ev.CA8subjets[i][j];
					jet_for_btagging j =  jet_for_btagging(this_sj.pt, this_sj.eta,this_sj.phi,this_sj.en,this_sj.M,this_sj.csv,"CA8subjet");
					this_ev.jets_for_btagging.push_back(j);
				}
			}
	...
	int max_N_of_candidate_nbjet=0;
  	for (Long64_t Nth = 0; Nth < Nev; Nth++)
		...
		vector<jet_for_btagging>  this_jets=events[Nth].jets_for_btagging;
		for (int i=0;i<this_jets.size();i++)
		{	
							
				jet_for_btagging this_j=this_jets[i];
				if (this_j.kind=="CA8jet"){R=0.8;}
				else {R=0.3;}
		...
		N_available_event++;
		N_of_candidate_nbjet=N_of_jet-N_of_candidate_bjet;
		...
		for (int i=0;i<this_jets.size();i++)
        	{
				...
				N_jet_this_ev++;
				if ( is_pass_pteta(this_j))
				{N_jet_pass_cut++;total_k++;}
				else
				{continue;}
				if ( btagging(this_j))
				{ N_btagging++;total_btag_pass_cut++;}
				else {total_nbtag_pass_cut++;}
	...
	cout<<"total_kk="<<gen_j<<"(b="<<gen_b<<", nb="<<gen_nb<<"),"<<endl;
	cout<<"total_k="<<gen_j_pass_cut<<"(b="<<gen_b_pass_cut<<", nb="<<gen_nb_pass_cut<<")"<<endl;
	...
	cout<<"total_kk="<<total_kk<<endl;
	cout<<"total_k="<<total_k<<endl;
	cout<<"total_btag_pass_cut="<<total_btag_pass_cut<<endl;
	cout<<"total_nbtag_pas_cut="<<total_nbtag_pass_cut<<endl;
	cout<<endl;
	...
	best_bcut_eff=results_cut2p[0].v;
	best_nbcut_eff=results_cut2p[1].v;
	...
	cout<<",best_bcut_eff="<<best_bcut_eff<<"+-"<<results_cut2p[0].err<<endl;
	cout<<",best_nbcut_eff="<<best_nbcut_eff<<"+-"<<results_cut2p[1].err<<endl;
	...
	cout<<",best_bcut_eff/true_bcut_eff="<<best_bcut_eff/true_bcut_eff<<endl;
	cout<<",best_nbcut_eff/true_nbcut_eff="<<best_nbcut_eff/true_nbcut_eff<<endl;
