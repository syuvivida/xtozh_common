...
void fcn_btagging_data(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
	...
	double total_log_like=0;
		...
		// par[0] is eff, par[1] is mr
		if(get_expvalue_of_Nt( par[0], par[1] ,t)==0){continue;}
		double this_log_like=log_poisson(double(Nt_from_data[t]), get_expvalue_of_Nt( par[0], par[1] ,t));
                total_log_like += this_log_like;
		if (this_log_like < -10000000)
		{ 
			cout<<"this_log_like < -10000000"<<"when eff="<<par[0]<<",mr="<<par[1]<<",t="<<t;
			cout<<",Nt_from_data[t]="<<double(Nt_from_data[t])<<",get_expvalue_of_N="<<get_expvalue_of_Nt( par[0], par[1] ,t)<<endl;
		}  
        }
...
vector<par_err_pair_for_minuit> get_min_par_btagging2p_data(vector<par_for_minuit> pars_ )
{
	int Npar = pars_.size();
  	TMinuit *gMinuit = new TMinuit(Npar);  //initialize TMinuit with a maximum of 2 params
  	gMinuit->SetFCN(fcn_btagging_data); // fcn_btagging is the function to do minimization
...
void Btagging_in_8Tev()
{
	...
	*/
	...
	Nk_from_data = vector<Int_t>(50,0);
	Nt_from_data = vector<Int_t>(50,0);
	...
	//get btagging 2p for data
	// par_for_minuit(par_number,par_name, par_starting_value, step, min, max)	
	par_for_minuit eff_data(0,"eff_rec", 0.7, this_step ,0+this_step,1-this_step);
	par_for_minuit mr_data(1,"mr_rec", 0.1, this_step ,0+this_step,1-this_step);	
	vector<par_for_minuit> pars_btagging2p_data;
	pars_btagging2p_data.push_back(eff_data);
	pars_btagging2p_data.push_back(mr_data);
	vector<par_err_pair_for_minuit> results_btagging2p_data=get_min_par_btagging2p_data(pars_btagging2p_data);
	best_eff_data=results_btagging2p_data[0].v;
	best_mr_data=results_btagging2p_data[1].v;
