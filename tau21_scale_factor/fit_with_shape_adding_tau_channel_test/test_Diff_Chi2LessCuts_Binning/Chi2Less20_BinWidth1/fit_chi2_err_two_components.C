...
//  Int_t bin_background2 = background_pos2->GetXaxis()->FindBin(xx);
  ...
  Double_t br1 = (1-par[0])*background_pos1->GetBinContent(bin_background1);
...
//  double bkgerr2 = background_pos2->GetBinError(bin_background2);
  ...
  err = sqrt(
             (1-par[0])*(1-par[0])*bkgerr1*bkgerr1 + 
...
*/
  ...
  fit_result->GetXaxis()->SetRangeUser(50,400);
  data->GetXaxis()->SetRangeUser(50,400);
