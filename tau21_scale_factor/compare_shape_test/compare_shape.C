...
TFile  *f1 = new TFile("ttbar_signal_and_background_shape_normalize.root");
TFile  *f2 = new TFile("Wbb_background_shape.root");

// get the template(the shape)
TH1D *h_Signal = (TH1D*)f1->Get("h_had_top_mass_signal");
TH1D *h_Bkg1 = (TH1D*)f1->Get("h_had_top_mass_bk1");
TH1D *h_Bkg2 = (TH1D*)f1->Get("h_had_top_mass_bk2");

TH1D *h_Wbb = (TH1D*)f2->Get("h_had_top_mass_bk2");

// normalize h_Wbb
h_Wbb-> Sumw2();
double scale = 1.0/( h_Wbb->Integral() );
h_Wbb->Scale(scale);
...
h_Wbb->SetLineColor(7);
h_Wbb->Draw("same");

TH1D *h_Wbb_fill = (TH1D*)h_Wbb->Clone("h_Bkg2");
h_Wbb_fill-> Sumw2(false);
h_Wbb_fill->SetFillStyle(1001);
h_Wbb_fill->SetFillColorAlpha(7,0.2);
h_Wbb_fill->Draw("same");
