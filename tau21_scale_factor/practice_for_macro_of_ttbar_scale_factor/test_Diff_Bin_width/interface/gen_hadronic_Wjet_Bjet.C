...
void gen_hadronic_Wjet_Bjet(TreeReader &data, vector<Int_t> *gen_hadronic_Wjet_index, vector<Int_t> *gen_hadronic_Bjet_index
		      ){
  ...
  //-----------------------------------------------------------------------------------//
  // find hadronic W (top->W) whose decay products is quark 
...
// vector<Int_t> gen_lep_nu_index;
 ...
 for(Int_t i = 0; i < nGenPar; i++){
     ...
     // find quarks <- W 
     if ( (genParId[ genDa1[ i ] ] != 1) && (genParId[ genDa1[ i ] ] != -1) &&
          (genParId[ genDa1[ i ] ] != 2) && (genParId[ genDa1[ i ] ] != -2) &&
          (genParId[ genDa1[ i ] ] != 3) && (genParId[ genDa1[ i ] ] != -3) &&
          (genParId[ genDa1[ i ] ] != 4) && (genParId[ genDa1[ i ] ] != -4) &&
          (genParId[ genDa1[ i ] ] != 5) && (genParId[ genDa1[ i ] ] != -5) &&
          (genParId[ genDa1[ i ] ] != 6) && (genParId[ genDa1[ i ] ] != -6) ){continue;}

     //cout<<"find W"<<endl;

     W_index = i ;

     gen_hadronic_Wjet_index->push_back(W_index);
 }
/*
 if(gen_hadronic_Wjet_index->size()>0){ 
   for(int i=0;i<gen_hadronic_Wjet_index->size();i++)
   {cout<<"gen_hadronic_Wjet_index->at(i): "<<gen_hadronic_Wjet_index->at(i)<<endl;}
 }
*/
  //-----------------------------------------------------------------------------------//
  // find B-jet using such W found in above

   for(int j=0;j<gen_hadronic_Wjet_index->size();j++){// W index loop

   	int top_index = genMo1[ gen_hadronic_Wjet_index->at(j) ];
	int Bjet_index;
...
//	cout<<"genDa1[ Bjet_index ]: "<<genDa1[ Bjet_index ]<<endl;
//        cout<<"genDa2[ Bjet_index ]: "<<genDa2[ Bjet_index ]<<endl;
...
//        cout<<"genParSt[dauther_2_index]: "<<genParSt[dauther_2_index]<<endl;
        ...
        gen_hadronic_Bjet_index->push_back(Bjet_index);
