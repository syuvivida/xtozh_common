...
float MinChi2_lep_and_had_1stAND2ndTerm(TreeReader &data, int lep_mu_index, int lep_B_jet_index,
					    int lep_nu_index, int had_W_jet_index,
					    int had_B_jet_index,  vector<Float_t> *nu_eta,
					    double& First_term, double& Second_term ){
  ...
  // calculate chi2 of combination 
   ...
   //input value to TLorentzVector

   lep_mu.SetPtEtaPhiM(  muPt[lep_mu_index],
                         muEta[lep_mu_index],
                         muPhi[lep_mu_index],
                         muM[lep_mu_index]);

   lep_b_jet.SetPtEtaPhiE(CA8jetPt[lep_B_jet_index],
                          CA8jetEta[lep_B_jet_index],
                          CA8jetPhi[lep_B_jet_index],
                          CA8jetEn[lep_B_jet_index]);

   lep_nu.SetPtEtaPhiM(  pfMetCorrPt,
                         nu_eta->at(lep_nu_index) ,
                         pfMetCorrPhi,
                         0);

   lep_top = lep_nu + lep_mu + lep_b_jet;

   had_W_jet.SetPtEtaPhiE(CA8jetPt[had_W_jet_index],
                          CA8jetEta[had_W_jet_index],
                          CA8jetPhi[had_W_jet_index],
                          CA8jetEn[had_W_jet_index]);

   had_b_jet.SetPtEtaPhiE(CA8jetPt[had_B_jet_index],
                          CA8jetEta[had_B_jet_index],
                          CA8jetPhi[had_B_jet_index],
                          CA8jetEn[had_B_jet_index]);
   ...
   float had_top_mass_resolution = 16.49;
   float top_mass = 172.9;

   float first_term = pow( fabs( lep_top.M() - top_mass ) ,2)/pow(lep_top_mass_resolution,2);
   float second_term = pow( fabs( had_top.M() - top_mass ) ,2)/pow(had_top_mass_resolution,2);

   First_term = first_term;
   Second_term = second_term;

   chi2 = first_term + second_term ;
